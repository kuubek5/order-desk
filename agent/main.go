...
func main() {
	...
	if f, path, failed := openLog(); f != nil {
		// Аварійний вихід теж у файл: паніка Go пише напряму в stderr, а він
		// у windowsgui-процесі веде в нікуди — тобто кожен крах агента досі був
		// безслідним.
		captureStderr(f)
		// Лише файл, БЕЗ io.MultiWriter(os.Stderr, f). MultiWriter зупиняється
		// на першій помилці й далі по списку не пише, а в процесі без консолі,
		// якого піднімає планувальник, stderr — порожній дескриптор: запис у
		// нього падав, і до файлу не доходив жоден рядок (відтворено 10.09.26
		// запуском без успадкованих дескрипторів — файл є, порожній). Копія в
		// консоль не губиться: captureStderr і так спрямував stderr сюди ж.
		log.SetOutput(f)
		logPathUsed = path
...
func runServe(cfgPath string) {
	...
	// Трей — на ГОЛОВНІЙ горутині (цикл повідомлень Windows цього вимагає), і
	// саме тому його падіння забирало весь процес разом із сервером знімків.
	// Оболонка перетворює це на «трей зник, HTTP лишився» — гірше за іконку,
	// але незрівнянно краще за мертвий верстат у CRM.
	func() {
		defer func() {
			if rec := recover(); rec != nil {
				log.Printf("паніка в треї: %v — сервер знімків працює далі", rec)
			}
		}()
