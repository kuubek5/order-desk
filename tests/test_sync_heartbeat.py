...
signal for the queue sidebar (app/sync_heartbeat.py SyncHeartbeat / record_heartbeat
/ heartbeat_status / _mail_sync_tick / _sheet_sync_tick).
...
from app import sync_heartbeat as heartbeat_mod
...
def test_record_success_sets_ok_and_clears_error(isolated_heartbeats):
    ...
    heartbeat = heartbeat_mod.heartbeats["mail"]
...
def test_record_error_stores_safe_message(isolated_heartbeats):
    ...
    heartbeat = heartbeat_mod.heartbeats["sheet"]
...
def test_record_skip_advances_timestamp_but_keeps_prior_status(isolated_heartbeats):
    ...
    first_attempt = heartbeat_mod.heartbeats["mail"].last_attempt_at
...
def test_mail_tick_skips_and_leaves_heartbeat_untouched_when_unconfigured(
    ...
    assert heartbeat_mod.heartbeats["mail"] == SyncHeartbeat()
...
def test_sheet_tick_records_success(isolated_heartbeats, monkeypatch):
    ...
    assert heartbeat_mod.heartbeats["sheet"].status == "ok"
