...
def test_column_widths_follow_the_window():
    """Ширини стовпців черги мусять перераховуватись на зміну розміру вікна.

    Доти вони перераховувались ЛИШЕ на свапі `#queue-rows` (полл раз на 15 с,
    шпилька, фільтр). Оператор працює на півекрана, там збережені ширини
    стискаються до мінімумів; розгортає вікно на весь екран — і колонки
    лишаються в тих мінімумах, а перший стовпець розтягується на весь
    залишок. Таблиця виглядає «зрізаною», а перше ж клацання шпильки після
    цього читається як стрибок стовпців, хоча то просто доганяє свап
    (скарга з цеху 17.09.26).

    Друга половина тієї самої вади — масштаб, що тільки СТИСКАВ: після
    повернення на широкий екран пропорції вже нікуди було відновлювати.
    """
    js = (STATIC_JS / "queue.js").read_text(encoding="utf-8")

    assert re.search(
        r'window\.addEventListener\(\s*"resize"', js
    ), "queue.js більше не слухає resize — ширини знову застигнуть на старому розмірі вікна"

    # Друга половина: перерахунок мусить брати ЗБЕРЕЖЕНУ мапу, а не поточні
    # пікселі. Інакше одне стиснення до мінімумів на півекрана затирає
    # пропорції назавжди, і жоден resize їх уже не відновить.
    apply_body = js.split("function applySavedWidths()", 1)[1].split(chr(10) + "  }", 1)[0]
    assert "loadWidths()" in apply_body, (
        "applySavedWidths більше не читає збережену мапу — пропорції нікуди "
        "буде відновлювати"
    )
    # І множник лишається СТИСКАЮЧИМ: розтягування скасовувало б власне
    # перетягування оператора (стовпець 300px ставав 430px).
    factor = re.search(r"const factor = ([^;]+);", js)
    assert factor, "у queue.js немає множника масштабу ширин"
    assert "sum > avail" in factor.group(1), (
        "множник знову розтягує: перетягнутий стовпець розповзеться на "
        "широкому екрані"
    )
