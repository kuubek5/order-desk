...
def test_approval_pending_is_not_the_approved_form_of_the_same_word():
    """«Погодж-» у коментарях несе два протилежні значення.

    Усі рядки нижче — справжні коментарі з бази (10.09.26). Корінь «погодж»
    позначив би «на погодженні» і ті роботи, які вже погоджені й чекають
    фрезерування, — тобто сигнал був би інвертований.
    """
    from app.services.queue import is_approval_pending_comment

    for comment in (
        "погодження Садовий (робота за 06.07)",
        "На погоджені Дарда/Маньо(робота за 01/09)",
        "Погодження АТАМАСЬ(робота за 01/09)",
        "погодження",
        "погодження степанян",
        "погодження до завтра стефания",
        "погодження КльовА  (робота за 07/09)",
        "на погодженні",
        "не погоджено",
        "на узгодженні з лікарем",
        "на согласовании",
    ):
    ...
    for comment in (
        "погоджено",
        "погоджено (робота за 06.07)",
        "швидка, погоджено (робота за 20.07)",
        "погоджен, швидка (робота за 27/08)",
        "Якщо можно, На швидку будь ласка!) (Робота за 23/06)\n"
        "Погоджували платофрму з лікарем  Фреза 1,06!!",
        # Технік дописав «погоджено» поверх старого «погодження» — плашка знімається.
        "погодження Маньо, погоджено",
        "погодження отримано",
        "роботи погоджені",
        "покрити опаком",
        "",
