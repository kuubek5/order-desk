...
def test_cam_comment_icon_shown_with_comment_text_in_title_when_set():
    html = _render(_order(cam_comment="покрити опаком, я сам закрию"))

    assert "cam-comment-icon" in html
    assert 'title="покрити опаком, я сам закрию"' in html


def test_cam_comment_icon_absent_when_no_comment():
    html = _render(_order(cam_comment=None))

    assert "cam-comment-icon" not in html
