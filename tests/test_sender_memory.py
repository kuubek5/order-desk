...
# База — спільна фікстура `db_session` з tests/conftest.py.
...
def test_remember_then_lookup_round_trip_and_upsert(db_session):
    db = db_session
...
def test_remember_keeps_folder_when_new_accept_has_none(db_session):
    db = db_session
...
def test_remember_ignores_blank_name(db_session):
    db = db_session
    e = EmailMessage(uid="a", from_address="x@y.z", subject="s")
    db.add(e)
    db.commit()
