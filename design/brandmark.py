...
# Корінь — від розташування файлу, а не зашитим шляхом: проєкт лежить на
# мережевій шарі, і буква диска залежить від того, чи змонтована вона зараз.
...
# Ім'я іконки застосунку — ОДНЕ місце на весь файл. Саме через два різні
# написання (тут і в збірці) знак і відв'язався від exe після перейменування.
APP_ICON_NAME = "kuubmill.ico"
...
if __name__ == "__main__":
    ...
    #     Ім'я мусить збігатися з тим, що читають KuubMill.spec, KuubMill.iss
    #     і windows_launcher. При перейменуванні застосунку воно поїхало на
    #     kuubmill.ico всюди, КРІМ цього рядка — і конвеєр знака мовчки
    #     розірвався: скрипт писав orderdesk.ico, збірка читала kuubmill.ico,
    #     тож правка форми не доїжджала б ні до exe, ні до трею, ні до ярлика.
    #     Сторож проти повторення — tests/test_brandmark_paths.py.
    p = ROOT / "assets" / APP_ICON_NAME
