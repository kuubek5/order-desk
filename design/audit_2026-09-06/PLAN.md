...
| # | Статус | Крок |
|---|---|---|
| B.1 | ✅ 07.09.26 | Бекап охоплює всі 27 таблиць (було 13). Копія несе власний перелічник «таблиця → рядків»; після відновлення числа звіряються, розходження = відкат, а не напівжива база. Копія з новішого формату відхиляється замість тихої втрати таблиць. Сторож `tests/test_backup_covers_every_table.py` валиться на новій таблиці без рядка в списку. |
...
| B.6 | ⬜ | Автоматичний бекап перед КОЖНИМ оновленням (зараз копія лише ручна + місячна): застосунок сам робить повну копію в `backups/` перед запуском інсталятора. |
| B.7 | ⬜ | Запобіжник на масові стирання: не більше N рядків таблиці за годину без підтвердження (зараз запобіжник є лише на масову архівацію в базі). |
| B.8 | ⬜ ⚠ | Кнопка «Відновити рядок» просто в «Журналі синку» — з тексту запису видно, що стерли; лишилось дати дію в один клік. |
