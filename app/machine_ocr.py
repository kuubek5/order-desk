...
def find_progress_bar(image: Image.Image) -> Optional[ProgressBar]:
    ...
    # Щілина в 1-2 пікселі між заливкою й рамкою — це згладжування краю, а не
    # залишок роботи: на смузі ~128px один піксель ≈ 0.8%, тобто така різниця
    # нижча за роздільність самої смуги. Без цього завершена програма показувала
    # 99% замість 100% (бойовий випадок 03.09.26).
    if last_white - fill_right <= EDGE_SLACK:
        last_white = fill_right
