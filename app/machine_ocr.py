...
# ── Помилка на екрані верстата ──────────────────────────────────────────────
# Вимога власника 13.09.26: коли на верстаті помилка (зламана фреза, задовгий
# інструмент, hardware limit switch), треба лише ЗАФІКСУВАТИ факт і підсвітити
# на віджеті. Текст помилки не читаємо — він щоразу різний і нам не потрібен.
#
# Два покоління — два різні канали, бо помилка виглядає геть по-різному:
#
#   1. Нове покоління (CORiTEC 150i/250i) малює модалку ВСЕРЕДИНІ свого
#      застосунку: суцільний насичений ЧЕРВОНИЙ банер зі словом ERROR на всю
#      ширину діалога. Окремого вікна ОС немає, тож ловиться лише з кадру.
#   2. RemiCORE (350i) показує звичайний діалог Windows із заголовком «Error».
#      Заголовки вікон агент і так читає — там пікселі не потрібні, а слово в
#      заголовку надійніше за будь-який розбір картинки.
#
# Чому саме СМУГА, а не слово «ERROR». Слово довелось би вчити як заголовок, а
# воно стоїть не в смузі заголовка, а посеред екрана, і його положення залежить
# від висоти діалога. Банер же має ознаку, якої більше ніде в цих інтерфейсах
# немає: широкий суцільний ЧЕРВОНИЙ рядок.
#
# Обидва пороги взято з ВИМІРУ 13.09.26, і другий із них — з пастки. По ширині
# рядка поділ чистий: на всіх 22 здорових бойових кадрах жоден рядок не дотягує
# до 0.30, а банер ERROR на 150i займає 0.57. Але самої ширини МАЛО. Кадр 350i
# (RemiCORE) дав суцільну смугу 20 px, де рядки справді червоні на 0.36 — і це
# НЕ помилка, а сітка інструментів, у якій кілька смуг прострочення стоять в
# один ряд. Тобто верстат із багатьма простроченими фрезами зчитувався б як
# «помилка» щохвилини. Рятує ТОВЩИНА: банер 150i — 98 px (0.082 висоти), а той
# ряд смуг — 0.023. Поріг 0.04 стоїть посередині, з дворазовим запасом на
# обидва боки. Саму помилку RemiCORE ловить інший канал — заголовок вікна.
ERROR_BANNER_MIN_WIDTH = 0.30    # частка ширини рядка, зайнята червоним
ERROR_BANNER_MIN_HEIGHT = 0.04   # частка висоти кадру — товщина суцільної смуги
# Насичений червоний: канал R великий І помітно більший за обидва інші. Рожевий
# титул діалогу Windows цього не проходить (там усі три канали високі) — і добре,
# бо той діалог ловиться заголовком вікна, а не кольором.
ERROR_RED_MIN = 120
ERROR_RED_DOMINANCE = 60


def screen_has_error_banner(image: Optional[Image.Image]) -> bool:
    """Чи на кадрі є червоний банер помилки (модалка нового покоління).

    Помилка тут коштує дорого в обидва боки: пропустити — верстат стоїть із
    зламаною фрезою й ніхто не знає; вигадати — оператор біжить до справного.
    Тому вимагаємо ОБИДВІ ознаки одразу, ширину І товщину.

    Кадру немає — це «не знаємо», а не «помилки немає»: той самий принцип, що з
    мовчанням агента про заголовки."""
    import numpy as np
    ...
    red = (
        (r >= ERROR_RED_MIN)
        & (r - g >= ERROR_RED_DOMINANCE)
        & (r - b >= ERROR_RED_DOMINANCE)
    )
    wide = red.mean(axis=1) >= ERROR_BANNER_MIN_WIDTH
    if not wide.any():
        return False
    # Найтовща СУЦІЛЬНА смуга таких рядків: окремі широкі рядки трапляються й
    # на здоровому екрані, а банер — це десятки рядків поспіль.
    need = max(2, int(height * ERROR_BANNER_MIN_HEIGHT))
    longest = run = 0
    for is_wide in wide.tolist():
        run = run + 1 if is_wide else 0
        longest = max(longest, run)
    return longest >= need


# Заголовок вікна діалогу помилки RemiCORE. Рівність, а не входження: «Error»
...
def titles_have_error(titles) -> bool:
    """Чи серед вікон верстата є діалог помилки (RemiCORE/350i).

    `None` (агент не відповів) — це НЕ «помилки немає»: ми просто не знаємо,
    і стан чіпати не можна. Той самий принцип, що з прив'язкою програми."""
    if not titles:
        return False
    return any(str(t).strip().casefold() in _ERROR_WINDOW_TITLES for t in titles)
