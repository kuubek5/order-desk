...
SETTING_FIELDS = [
    ...
    # бачить, які диски взяли з архіву, і складає замовлення на склад — ту
    # саму роботу, яку робили обходом шухляд. Читання-лише.
    # operator_editable: це шлях на конкретній машині, а не секрет — той
    # самий довід, що для export і Cam-work. Розділу налаштувань немає:
    # поле живе в нижній смузі екрана «Нові диски» (/discs/path).
    SettingField(
        key="cam_blanks_path",
...
PREFERENCE_KEYS = {
    ...
    # Друга папка «оброблено» — куди лист іде після фрезерування («Відфрезеровано»,
    # власник 25.09.26). Звірка скриньки вважає лист у ній так само обробленим, а
    # не «покинув Вхідні». Порожнє = другого етапу немає.
    "mail_milled_folder",
    "notify_style",
    "notify_position",
    "notify_events",
    ...
    # Табло пічок для логістів (app/services/furnace_board.py): окремий вхід
    # у мережі цеху на порту 8010, лише перегляд. enabled — "1"/""; token —
    # секрет у посиланні (не пароль до чогось, його свідомо роздають логістам;
    # змінюється кнопкою, і старе посилання перестає працювати).
    "furnace_board_enabled",
    "furnace_board_token",
    # Доступ до `/mcp` по мережі (app/services/mcp_gateway.py): окремий слухач
    # на порту 8011, лише читання. enabled — "1"/""; token — єдина межа
    # доступу, бо запит приходить з ІНШОЇ машини (тунель), а не з петлі. У
    # Jinja-контекст значення не віддається — екран знає лише «задано».
    "mcp_remote_enabled",
    "mcp_remote_token",
    # Робота з інших ПК (app/services/network_access.py): головний застосунок
    # слухає 0.0.0.0:8000 замість 127.0.0.1. "1"/"". Читається лаунчером РАЗ
    # на старті — зміна набуває чинності після перезапуску застосунку.
    "network_access_enabled",
    # Відкриття тек на ПК операторів (протокол kmill-folder://). "1"/"". Увімкнено
    # → сервер віддає мережевому клієнту протокол-посилання, і кнопка «Відкрити
    # папку» відкриває теку в Провіднику на ПК оператора (потрібен помічник).
    "network_folder_open",
...
NOTIFY_EVENTS: tuple[tuple[str, str, str, bool], ...] = (
    ...
    # ── Додано 08.09.26 разом зі стилем «Аврора» ──────────────────────────
    # «Можна брати» — це НЕ повернення прибраного `new_orders`. Той рахував
    # РОЗМІР черги (status != видано) і роздувався від будь-якого повернення
    # «видано»→«нове». Цей рахує стан готовності: job_code заповнений і Sum3D
    # порожній (§5 CLAUDE.md). Приріст = технік доклав шлях до папки, тобто
    # рівно та подія, якої чекає оператор. Хибно зрости може лише коли хтось
    # СТИРАЄ Sum3D у таблиці, а це і є свідоме повернення роботи в чергу —
    # оператор має про нього знати.
    ("ready_to_take", "Роботу можна брати (технік здав)", "info", True),
    # Рядок зник із таблиці. `archived_at` ставиться ЛИШЕ коли рядок не
    # знайшовся у вкладці (sync.py:955) — retention нічого не штампує, він
    # фільтрує чергу за датою. Тому приріст = хтось прибрав рядок, а не
    # «робота постаріла».
    ("row_deleted", "Рядок видалено з таблиці", "warn", True),
)
NOTIFY_EVENT_KEYS = {key for key, _, _, _ in NOTIFY_EVENTS}
# «aurora» — вигляд 08.09.26: картка-скло для подій ззовні, нижня стрічка для
# підтверджень власних дій, світна кромка для збоїв. «glass» і «card»
# лишаються доступними і працюють точно як раніше (одна колонка тостів).
NOTIFY_STYLES = {"glass", "card", "aurora"}
