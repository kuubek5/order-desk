...
// ── Пульс видачі: відбиток дня живе В БРАУЗЕРІ ─────────────────────────
//
// Кожні 15 с прихований елемент питає `/handout/pulse` — і питає З ВІДБИТКОМ
// стану, який ЦЕЙ екран показує зараз. Сервер порівнює зі своїм і, якщо день
// змінився, віддає `HX-Trigger: refresh-handout`; картки перемальовуються раз.
//
// Доти відбиток жив на сервері в словнику за (користувач, день) — і другий
// комп'ютер не оновлювався ніколи: перший пульс, що приходив після зміни,
// забирав її собі й лишав у словнику вже свіжий відбиток, тож для другого
// браузера «нічого не змінилось» (скарга з цеху 17.09.26). Стан екрана мусить
// жити в тому екрані.
//
// Порядок списку від оновлення НЕ міняється: картки сортуються за позицією в
// таблиці (правило видачі №1).
document.body.addEventListener("htmx:configRequest", (event) => {
  const el = event.detail && event.detail.elt;
  if (!el || !el.matches || !el.matches("[data-handout-pulse]")) return;
  // Перший пульс після завантаження бере відбиток зі сторінки: якщо день
  // змінився між рендером і першим тіком, екран це побачить одразу.
  event.detail.parameters.fp = window.kmHandoutFp || el.dataset.fp || "";
});

// Відбиток приїжджає і з пульсу, і з нових карток — тому слухаємо будь-яку
// відповідь. Картки оновлюють його в тій самій відповіді, у якій приходять,
// інакше оператор після власної галочки ловив би зайве перемальовування.
document.body.addEventListener("htmx:afterRequest", (event) => {
  const xhr = event.detail && event.detail.xhr;
  if (!xhr || !xhr.getResponseHeader) return;
  const fp = xhr.getResponseHeader("HX-Handout-Fp");
  if (fp) window.kmHandoutFp = fp;
});
