...
(function () {
  ...
  function applySavedWidths() {
    ...
    //
    // Множник ТІЛЬКИ стискає, і це навмисно. Розтягувати не можна: оператор,
    // який звузив стовпець до 300 px, отримав би 430 на широкому екрані —
    // його власне перетягування виглядало б скасованим (зловлено заміром
    // 17.09.26 на живій копії). Ширший за суму екран лишає вільне місце, як
    // і лишав. Полагодити «зрізану» таблицю після розгортання вікна мусить
    // не множник, а перерахунок у потрібний момент — див. слухач `resize`
    // нижче: пропорція завжди береться зі ЗБЕРЕЖЕНОЇ мапи, тож стиснення до
    // мінімумів на півекрана її не затирає.
  ...
  // Зміна розміру вікна мусить перерахувати ширини ОДРАЗУ. Доти вони
  // перераховувались лише на свапі `#queue-rows` (полл, шпилька, фільтр), тож
  // між двома тіками таблиця лишалась у геометрії попереднього розміру вікна:
  // розгорнув на весь екран — колонки стоять у мінімумах від півекрана, а
  // перший стовпець з'їв увесь залишок. І навпаки: перше ж клацання шпильки
  // після зміни розміру виглядало як стрибок стовпців, хоча шпилька тут ні
  // до чого — це доганяв свап.
  //
  // Дебаунс, бо `resize` б'є десятками подій за одне перетягування рамки, а
  // перерахунок торкається кожного заголовка.
  let widthsTimer = null;
  window.addEventListener("resize", () => {
    if (widthsTimer) clearTimeout(widthsTimer);
    widthsTimer = setTimeout(applySavedWidths, 120);
  });
  document.querySelectorAll("[data-layout-edit-toggle]").forEach((btn) => {
