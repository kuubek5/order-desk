...
def sync_tab(
    ...
    for row in rows:
        ...
        if existing is None:
            # Мʼяка ознака «не наша робота» вище пропускає рядок, коли на його
            # позиції ВЖЕ стоїть відома робота: інакше технік, що на секунду
            # стер матеріал, гасив би живу роботу. Але пропуск діяв і далі — на
            # СТВОРЕННЯ. Коли рядок переписали під інший вміст, стара робота
            # йшла в архів, а на її місці народжувалась нова — і в ній могло
            # бути що завгодно. Бойовий випадок 17.09.26: у вкладці 16.09 рядок
            # 147 став «CADCAM Команда» з матеріалом «9» і порожньою кількістю
            # (число, вписане в чужу колонку) — і поїхав на ранкову видачу
            # окремим клієнтом.
            #
            # Створення — це ПЕРША зустріч із цим вмістом, тут ознака мусить
            # діяти так само, як на новому рядку. Захист живої роботи від
            # правки поля лишається: він про ОНОВЛЕННЯ, і сюди не заходить.
            if _is_non_queue_row(row, row_fills):
                result.skipped_non_queue += 1
                continue
