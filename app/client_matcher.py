...
# ── Кеш зіставлення ─────────────────────────────────────────────────────────
# `match_client_name` — чистий CPU: нормалізація, транслітерація і скорер по
# шортлісту тек, на КОЖНОГО клієнта екрана. На бойових замірах 09.09.26 саме
# він був найдорожчим на видачі — `match:clients` 0.46-1.08 с у КОЖНОМУ запиті,
# більше за все інше разом, включно з обходом мережевої шари (0.00-0.04 с).
# А платили за нього щоразу заново: кожна галочка «знайдено» перебудовує екран
# і перезіставляє тих самих клієнтів із тими самими теками.
#
# Функція чиста, тож результат можна тримати. Ключ описує ВЕСЬ вхід — перелік
# тек і словник підтверджених псевдонімів, — тому нова тека в `export` або
# щойно підтверджений псевдонім самі роблять старі записи недосяжними. Це
# важливіше за економію: кеш, який переживає зміну входу, показував би вчорашнє
# зіставлення й відправляв оператора до чужої теки.
_MATCH_CACHE_MAX = 4096
_match_cache: dict[tuple, MatchResult] = {}


def matcher_cache_key(folder_names, known_aliases) -> tuple:
    """Відбиток входу для `match_client_name_cached`.

    Рахується ОДИН раз на запит, а не на кожного клієнта: перелік тек однаковий
    для всього екрана, і хешувати його двадцять разів було б тією самою
    економією навпаки.
    """
    return (tuple(folder_names), tuple(sorted(known_aliases.items())))


def match_client_name_cached(
    sheet_name: str,
    folder_names: list[str],
    known_aliases: dict[str, str],
    cache_key: tuple,
) -> MatchResult:
    """`match_client_name` з памʼяттю на (імʼя + відбиток входу).

    `cache_key` приходить ззовні свідомо: інакше кожен виклик будував би
    відбиток заново, і на цьому згоріла б уся економія. Розплата — виклик
    зобовʼязаний віддавати відбиток ТИХ САМИХ `folder_names` і `known_aliases`,
    які передає; звідси й `matcher_cache_key` поруч.
    """
    key = (sheet_name, cache_key)
    hit = _match_cache.get(key)
    if hit is not None:
        return hit
    result = match_client_name(sheet_name, folder_names, known_aliases)
    # Межа проти нескінченного росту: кожна нова тека в `export` робить НОВИЙ
    # відбиток, тобто новий набір ключів, а старі стають мертвими. Раз на
    # переповнення чистимо все — це дешевше за облік віку кожного запису, а
    # втрата кеша коштує один перерахунок.
