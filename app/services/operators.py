...
def validate_self_registration(
    ...
    * ім'я в базі не буває порожнім: якщо його не ввели, воно дорівнює логіну,
      бо з нього ростуть історія дій, список операторів і підказки;
    * зайнятий логін віддає ту саму помилку, що й будь-яка інша, і НЕ
      підтверджує, що такий користувач існує.
    """
    login = username.strip()
    # Імені у формі більше немає (рішення власника 08.09.26): оператор вводить
    # лише логін. Але `full_name` — не окраса: воно показується в історії дій,
    # у списку операторів і в підказках, тож порожнім бути не може. Тому за
    # замовчуванням дорівнює логіну, а адмін за потреби виправить у
    # Налаштуваннях. Ім'я, передане ззовні (майбутня форма чи CLI), поважаємо.
    values = {
        "username": login,
