...
def answered_from_verdict(
    ...
    Звіряємо з ГОЛОВОЮ вироку (`machines.note_head`), а не з повним текстом:
    пояснення після голови вже переписувалось (10.09.26), а в журналі лежать
    рядки зі старим. Будівник той самий, що пише вирок, — не копія рядка тут.
    За входженням, не за рівністю (урок `classify`). Не впізнали — None:
    вгадувати відповідь з чужого тексту не будемо.
    """
    if not verdict:
        return None
    from app.services import machines as _m
    ...
    for answer in (True, False):
        head = _m.note_head(answer, host, port)
        if head and head in verdict:
            return answer
    return None
...
def explain(
    *,
    ...
    if cause == CAUSE_AGENT_DOWN:
        ...
        if answered is True:
            proof.append(
                f"Стук у сусідні порти: ПК {host} озвався. Отже він увімкнений і "
                f"в мережі, а мовчить саме порт {port}."
            )
            # Раніше тут був однозначний вирок «брандмауер або профіль
            # мережі», і він збрехав на першому ж розборі (10.09.26): агент
            # 150i і 250i падав, правило брандмауера було в порядку. На порт
            # без слухача брандмауер Windows не відповідає відмовою, а мовчить
            # — мертвий агент і закритий порт ззовні однакові.
            doubt.append(
                "Звідси не розрізнити, чи агент на ПК не працює, чи порт "
                "закриває брандмауер: на порт, де ніхто не слухає, брандмауер "
                "Windows не відповідає відмовою, а мовчить — так само, як на "
                "закритий."
            )
            actions += [
                f"На ПК {host} у PowerShell: Get-Process kmill-agent | "
                "Select-Object StartTime. Час старту ПІСЛЯ обриву — агент "
                "падав, і його підняв сторож; тоді дивитись лог агента.",
                f"Якщо агент працює давно — перевірити правило вхідних для "
                f"порту {port} у брандмауері.",
            ]
