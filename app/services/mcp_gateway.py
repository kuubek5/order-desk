...
def connect_links(db: Session) -> list[str]:
    """Готові рядки «адреса + токен», які достатньо скопіювати й передати.

    Навіщо токен прямо в адресі. Перший варіант цього екрана показував токен
    РІВНО один раз і окремо від адреси, а власник мусив зліпити їх докупи й
    не загубити. На практиці це означало «запиши зараз, бо більше не
    побачиш» — і зайвий крок там, де кроків і так вистачає. Тут рядок
    складений, видимий щоразу й копіюється кнопкою; слухач приймає токен і з
    `?t=`, і з заголовка.

    Межа від цього не зсунулась. Екран налаштувань і так лише для адміна й
    лише з цього компʼютера, а хто сидить за ним, той однаково читає файл бази
    поруч. Рівно так само живе посилання табло печей
    (`furnace_board.board_links`) — не заводимо другого правила для того
    самого класу секрету.
    """
    token = gateway_token(db)
    if not token:
        return []
    return [f"http://{address}:{GATEWAY_PORT}/mcp?t={token}" for address in gateway_addresses()]


def firewall_command() -> str:
    """Команда, що дозволяє вхідні на порт слухача — одним рядком, на всі профілі.

    Чому це дається готовим рядком, а не інструкцією «відкрийте брандмауер».
    Правило, написане лише для профілю «Приватна», нічого не блокує, але й
    нічого не дозволяє — а інтерфейс WireGuard Windows майже завжди
    класифікується як «Загальнодоступна». Та сама пастка вже коштувала місяць
    хибного діагнозу обривів верстатів (CLAUDE.md §14). `netsh` без `profile=`
    застосовує правило до ВСІХ профілів, тож вибирати нічого не треба й
    помилитись ніде.
    """
    return (
        'netsh advfirewall firewall add rule name="KuubMill MCP" '
        f"dir=in action=allow protocol=TCP localport={GATEWAY_PORT}"
    )
