...
class MachineState:
    ...
    # Верстат СТОЇТЬ, і ми це ЗНАЄМО: відомий екран простою (SELECT JOBS,
    # WINDOWS UPDATES — рішення власника 13.09.26). На відміну від «нічого не
    # прочитали», це позитивний сигнал: він дає «стоїть» замість «не читається»
...
def poll_target(
    ...
    if sisma is not None:
        ...
        validating = idle_known = False
    else:
        # ОДИН розбір заголовка на ВСІ відомі екрани: маска в них спільна, а кадр
        # 1920×1200 переганяти в масив на кожен екран окремо — зайва робота на
        # кожному тіку кожного верстата. Значення заголовка:
        #   done  — SUMMARY (обидва покоління): програма завершена;
        #   check — VALIDATE JOBS: верстат ПЕРЕВІРЯЄ програму. Число з нього НЕ
        #           беремо свідомо — внизу там своя смуга на всю ширину кадру, і
        #           вона рахує перевірку, а не фрезерування; показати її як
        #           прогрес означало б сказати «робота на 56 %», коли робота ще
        #           не почалась. Сьогодні жоден читач ту смугу й не бачить (біла
        #           доріжка), але правило має стояти в коді;
        #   idle  — SELECT JOBS / WINDOWS UPDATES: верстат СТОЇТЬ, і ми це
        #           ЗНАЄМО (рішення власника 13.09.26). Позитивний сигнал: знімає
        #           кадр зі скриньки невідомих і дає «стоїть», а не «не читається».
        try:
            meaning = screen_meaning(frame)
        except Exception:  # noqa: BLE001 — читання кадру не має валити опитування
    ...
    try:
        ...
        fault = False
    # Коли екран НАЗВАВ себе, його слово старше за будь-яке число з геометрії.
    # Перевірка (`check`) відкидала відсоток і раніше; простій (`idle`) мусить
    # так само, і це не теорія: на екрані SELECT JOBS 150i читач намалював «0 %»
    # з порожнього місця (справжня смуга не знайдена — `find_progress_bar` там
    # None, число дав геометричний читач нового покоління). Далі 0 — це не None,
    # тож `state_key` віддавав «фрезерує 0 %», і верстат, який СТОЇТЬ із порожньою
    # текою завдань, висів окремим станом серед решти, що чесно показували
    # «завершено» (скарга власника 14.09.26). На екрані вибору файлу програми
    ...
    if (
        ...
        # SUMMARY, ні відомого екрана простою, ні назви програми. Це не
        # обовʼязково поломка — так виглядає і робочий стіл, — але саме такі
        # екрани й треба один раз побачити й назвати. Дублі бере на себе
        # відпечаток, а «це неважливо» знімає питання назавжди. Відомий екран
        # простою (`idle_known`) сюди не потрапляє: він уже впізнаний.
        screen_inbox.note(
            db,
            kind=screen_inbox.KIND_MACHINE,
            key=target.key,
...
class MachineCard:
    ...
    def is_running(self) -> bool:
        """Програма йде — ТИМ САМИМ станом, що й усі підписи.

        Раніше тут був власний вираз «є відсоток і він менший за 100», і він
        розходився з `state_key` в обидва боки. Шапка віджета рахувала
        «2 фрезерує», зараховуючи верстат, чия ж таки плитка поруч писала
        «програма не йде» (нуль без завантаженої програми), — тобто підсумок
        сперечався з тим, що під ним (скарга з цеху 15.09.26). Два ПОГЛЯДИ на
        один стан — нормально, два ПРЕДИКАТИ — ні; це той самий урок, що з
        `open_notes()`/`open_note_count()`.
        """
        return self.state_key == "run"
    ...
    @property
    def is_idle_known(self) -> bool:
        """Відомий екран простою (SELECT JOBS, WINDOWS UPDATES): верстат стоїть,
        і ми це знаємо. Читається зі СВІЖОГО кадру, як і решта станів — «стоїть»
        із протухлого кадру нічого не доводить."""
        if not self.state or self.stale or self.has_problem:
            return False
    ...
    def state_key(self) -> str:
        ...
        if self.has_program:
            return "busy"
        # Відомий екран простою — «стоїть» напевно, навіть якщо відсотка від
        # цього верстата ми ще не бачили: інакше він показав би «не читається».
        if self.is_idle_known:
            return "idle"
        # Кадр є, ні відсотка, ні програми, ні відомого екрана простою. Що це
        # означає — залежить від того, чи вміємо ми взагалі читати екран САМЕ
        # цього верстата.
    ...
    def state_note(self) -> str:
        ...
        return {
            # Без заклику до дії (рішення власника 13.09.26): плитка називає
            # стан, а не командує. «Підійдіть» до того ж і неправда в половині
            # випадків — оператор уже може стояти біля верстата.
            "fault": "на екрані повідомлення про помилку",
