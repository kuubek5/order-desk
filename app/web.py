...
def _client_pane_context(db: Session, client: Client, named_orders: list[Order] | None = None) -> dict:
    ...
    return {
        "client": client,
        "summary": summary,
        "units_total": sum(_quantity_units(o.quantity) for o in matched),
        "aliases": aliases,
...
@app.get("/clients/{client_id}/pane", response_class=HTMLResponse)
def get_client_pane(request: Request, client_id: int, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse(
        request, "_client_pane.html", {"user": user, **_client_pane_context(db, client)}
    )
...
def bind_client_folder(
    ...
    if request.headers.get("HX-Request") == "true":
        ...
        return templates.TemplateResponse(
             ...
             **_client_pane_context(db, client)},
        )
