...
# Текст один на всі місця, де ми відмовляємось чіпати файли листа, поки
# покинутий фетч ще качає в ту саму теку. Однакове формулювання важливе: у
# синку й на кнопці листа це ОДНА ситуація, і оператор має впізнавати її.
ZOMBIE_FETCH_MESSAGE = (
    "Пошта ще довантажує файли попередньої перевірки — зачекайте кілька хвилин."
)


def zombie_fetch_blocks_files() -> str | None:
    """Причина, чому зараз НЕ можна чіпати файли вкладень, або None.

    Фоновий синк цей гейт мав (`run_sync`), а ручні кнопки в листі — ні:
    «Скачати файли», «Скачати наново», «Забрати за посиланням» і навіть
    «Прийняти» працювали з тим самим листом, поки покинутий фетч качав у ту
    саму теку своєю сесією. Наслідок — подвійні вкладення з суфіксом «(2)», а
    для приймання гірше: воно переносило файли з-під зомбі, і його коміт лишав
    рядки з мертвими шляхами (аудит 08.09.26).
    """
    return ZOMBIE_FETCH_MESSAGE if zombie_fetch_running() else None
