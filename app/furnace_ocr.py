...
ZONES: dict[str, Zone] = {
    ...
    # Ліва межа — 284, а не 281: на 281–282 стоїть САМА РАМКА поля (виміряно
    # на кадрі печі 17.09.26 — суцільна світла лінія заввишки на всю висоту
    # поля). Рамка потрапляла в зону, сегментатор рахував її за символ, той не
    # впізнавався — і читання відкидалось цілком: сире «?C0» замість «C0».
    # Мовчало так із 12.09.26, 5924 кадри поспіль. Права межа (404–405) сюди й
    # не входила, тому й помітно було лише зліва.
    "command": Zone(
        (284, 421, 402, 441), "light", "Команда",
        r"^(?=.*[A-Z])(?=.*\d)[0-9A-Z.]{2,12}$",
    ),
...
def read_zone(panel: Image.Image, name: str) -> Field:
    ...
    if clipped:
        ...
        # Ключ — САМА ЗОНА, без розпізнаного тексту. З текстом у ключі глушник
        # був би марний: обрізаний символ читається щоразу трохи інакше
        # («?C0», «?CO», «?C8»), кожне нове значення вважалось би новою подією,
        # і лог сипався б так само, як до глушника. Сам текст лишається в
        # повідомленні — він потрібен для діагностики, але не для тотожності.
        skipped = log_throttle.due(f"furnace.clipped:{zone.title}")
