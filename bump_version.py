#!/usr/bin/env python
"""Підняти версію застосунку в усіх місцях за один крок.

Навіщо: версія зашита руками в app/__version__.py та installer/KuubMill.iss, а
CHANGELOG.md ще й чекає штампа «номер + дата» на розділі незалежних змін. У
минулих сесіях це робилось `sed`-ами — легко проґавити одне місце й отримати
провал релізу. Див. SESSIONS_DIAGNOSTICS.md, кластер 10.

.github/workflows/release.yml НЕ чіпається: версію він бере з тега
(`github.ref_name`), зашитого рядка там більше немає. Прохід по ньому лишився
як страховка на випадок, що рядок повернуть, тому «release.yml: 0 замін» у
виводі — норма, а не збій.

ТЕКСТ журналу змін пишеться РУКАМИ до запуску цього скрипта — сюди приїжджає
лише заголовок. Порожній розділ = безмовний реліз, і ніщо на це не свариться.
...
Після бампу:
    ...
    git commit (разом із CHANGELOG.md) + git tag v0.3.1 + git push origin v0.3.1
"""
